...
export default function TopicoDetalhe() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const { data: postData, error: postError } = await supabase
        .from('posts')
        .select('id, title, content, author_name, created_at, likes, dislikes, communities(name, slug)')
        .eq('id', postId)
        .single();
  ...
  async function handlePostLike() {
    ...
    const newDislikes = userPostVote === 'dislike' ? Math.max(0, postDislikes - 1) : postDislikes;
  ...
  async function handlePostDislike() {
    ...
    const newLikes = userPostVote === 'like' ? Math.max(0, postLikes - 1) : postLikes;
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-neutral-950 text-neutral-100 p-6 max-w-3xl mx-auto flex items-center justify-center">
        <p className="text-neutral-400">Carregando discussão...</p>
      </main>
    );
  }
  ...
  if (!post) {
    return (
      <main className="min-h-screen bg-neutral-950 text-neutral-100 p-6 max-w-3xl mx-auto">
        <p className="text-neutral-400">Tópico não encontrado.</p>
        <Link className="text-emerald-400 hover:text-emerald-300 hover:underline mt-4 inline-block" href="/">
          ← Voltar ao feed
        </Link>
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-neutral-950 text-neutral-100 p-4 sm:p-6 max-w-3xl mx-auto">
      {/* Cabeçalho */}
      <header className="mb-6 border-b border-neutral-800 pb-4 flex justify-between items-center gap-4">
        <Link className="text-sm text-neutral-300 hover:text-white transition" href="/">
          ← Voltar ao Feed
        </Link>
        ...
        <div className="space-y-4 mb-8">
          {comments.length > 0 ? (
            comments.map((comm) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 pt-3 border-t border-neutral-700">
                  ...
                  <button
                    type="button"
                    onClick={() => handleCommentVote(comm.id, 'like')}
                  ...
                  <button
                    type="button"
                    onClick={() => handleCommentVote(comm.id, 'dislike')}
                    aria-label="Não gostei do comentário"
                    className={`inline-flex items-center justify-center gap-2 min-h-[38px] px-3 rounded-lg border-2 text-xs font-bold transition-all cursor-pointer ${
                      votedComments[comm.id] === 'dislike'
            ...
            <p className="text-sm text-neutral-400">
              Nenhum comentário até agora. Seja o primeiro a responder!
            </p>
          )}
        </div>
